...
datafile = "Model-III-high-dataset"
...
scatter(0:105,Itrue[1:106], title = "Model 3: SafeBlues Universal ODE SIR", xaxis = "Day", yaxis = "Percentage Infected", label = "True infected", color =:red)
...
plot!([80-0.01,80+0.01],[0.0,0.03],lw=3,color=:black,label="Training Data End")

savefig("MSaratnn_RD_UODE_SB_Traj2_Train81_Policy91.pdf")

using JLD
save("MSarat_RD_SafeBlues_Traj2_Train81_Policy91.jld", "Itrue", Itrue , "Isb", Isb, "sol_sb_2", sol_sb[2, 1:106], "sol_sb_5", sol_sb[5, 1:106], "p", res.minimizer)

#EXTRAPOLATION UNDER DIFFERENT POLICY DECISIONS

D = load("MSarat_RD_SafeBlues_Traj2_Train81_Policy91.jld")
Itrue = D["Itrue"]
Isb = D["Isb"]
sol_sb_2 = D["sol_sb_2"]
sol_sb_5 = D["sol_sb_5"]
...
policy_amount_medium = [0.2,0.3,0.4,0.4]
...
plot!(81:105,sol_sb_low[2,:]./10^5,lw=3, label = "Predicted: Strong lockdown", color = :red)
plot!(81:105,sol_sb_medium[2,:]./10^5,lw=3, label = "Predicted: Moderate lockdown", color = :green)
plot!(81:105,sol_sb_high[2,:]./10^5,lw=3, label = "Predicted: Weak lockdown", color = :purple)
plot!([80-0.01,80+0.01],[0.0,0.03], ylims = (0, 0.11), lw=3,color=:black,label="Training Data End")
savefig("MSaratn_Social_Dist_Policy_Variation_Train81_policy91n.pdf")
...
R_eff_0_6 = zeros(Float64, length(t), 1)
...
policy_amount_0_6 = [0.2,0.3,0.6,0.6]
...
for i = 1:length(R_eff_low)
  ...
  δ = p[1]
  δk= p[2]
  num[i] = 4*ann([policy_amount_0_6[ind]],p[3:end])[1]*δ
  den[i] = δk*ann([policy_amount_0_6[ind]],p[3:end])[2]
  R_eff_0_6[i] = num[i]/den[i]
...
D = load("MSarat_RD_R0_SafeBlues_Traj2.jld")
tn = D["t"]
R_eff = D["R_eff"]

Policy = [1 ;0.9; 0.8; 0.7; 0.6; 0.5; 0.4; 0.3;  0.2]
R_model1 = [R_eff_low[93]; R_eff_0_2[93]; R_eff_0_3[93]; R_eff_0_4[93]; R_eff_0_6[93]; R_eff_0_7[93]; R_eff_medium[93]; R_eff_0_9[93]; R_eff_high[93] ]

using LaTeXStrings
plot(Policy,R_model1, framestyle = :box, title = "Effect of lockdown on forecasted reproduction number", ylims = (0, 5), xlims = (0, 1.2),  legend = :topright, xaxis = "Lockdown strength (l)", yaxis = L"R_{\textrm{eff}}(t)", label = "Model3", linewidth = 3, color = :black)

savefig("MLine_All_Policy_MSaratn_R0_Social_Dist_Policy_Variation_Train81_Policy91.pdf")
